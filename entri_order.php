    ...
    <div class="row-fluid">
                  ...
                  ?>
                    ...
                    <?php
                      ...
                      if (isset($_REQUEST['tambah_pesan'])) {
                          // Validasi data
                          if (isset($_REQUEST['tambah_pesan']) && is_numeric($_REQUEST['tambah_pesan'])) {
                              ...
                              // Query pertama: Masukkan data ke tabel tb_pesan
                              $p = isset($_POST['id_pesan']) ? $_POST['id_pesan'] : 0;
                              $o = isset($_POST['id_order']) ? $_POST['id_order'] : 0;
                              $s = isset($_POST['jumlah']) ? $_POST['jumlah'] : 0;
                              $t = isset($_POST['status_pesan']) ? $_POST['status_pesan'] : '';


                              $query_tambah_pesan = "INSERT INTO tb_pesan (id_pesan, id_user, id_order, id_masakan, jumlah, status_pesan) VALUES ('$p', '$id', '$o', '$id_masakan', '$s', '')";
                              ...
                              if (!$sql_tambah_pesan) {
                                  ...
                                  $id_pesannya = mysqli_insert_id($conn);

                                  // Query kedua: Masukkan data ke tabel tb_stok
                                 $query_olah_stok = "INSERT INTO tb_stok (id_pesan, jumlah_terjual, status_cetak) VALUES ($id_pesannya, 0, 'belum cetak')";
                              ...
                              if ($success) {
                                  ...
                                  echo "Gagal menambah pesanan."; // Pesan kesalahan
                              }

                              // Aktifkan kembali otomatisasi commit
      ...
      <?php
          ...
          if(isset($_POST['proses_pesan'])){
            $o = isset($_POST['id_order']) ? $_POST['id_order'] : '';
            $id_admin = isset($r['id_admin']) ? intval($r['id_admin']) : 0;
            
            $id_pengunjung = $id;
            $no_meja = isset($_POST['no_meja']) ? $_POST['no_meja'] : '';
            $total_harga = isset($_POST['total_harga']) ? $_POST['total_harga'] : '';
            $uang_bayar = isset($_POST['uang_bayar']) ? $_POST['uang_bayar'] : '';
            $uang_kembali = isset($_POST['uang_kembalian']) ? $_POST['uang_kembalian'] : '';
            $status_order = 'belum bayar';
            date_default_timezone_set('Asia/Jakarta');
            $time = Date('YmdHis');
            echo $time;
            $query_simpan_order = "INSERT INTO tb_order VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($query_simpan_order);
            $stmt->bind_param("sssssssss", $id_order, $id_admin, $id_pengunjung, $time, $no_meja, $total_harga, $uang_bayar, $uang_kembali, $status_order);
            $stmt->execute();
        
            $query_tampil_order = "SELECT * FROM tb_order WHERE id_pengunjung = ? ORDER BY id_order DESC LIMIT 1";
            $stmt_tampil_order = $conn->prepare($query_tampil_order);
            $stmt_tampil_order->bind_param("s", $id);
            $stmt_tampil_order->execute();
            $result_tampil_order = $stmt_tampil_order->get_result();

            if ($row_tampil_order = $result_tampil_order->fetch_assoc()) {
                $id_ordernya = $row_tampil_order['id_order'];
            
                $query_ubah_jumlah = "SELECT * FROM tb_pesan 
                LEFT JOIN tb_masakan ON tb_pesan.id_masakan = tb_masakan.id_masakan WHERE id_user = ? AND status_pesan != 'sudah'";
                $stmt_ubah_jumlah = $conn->prepare($query_ubah_jumlah);
                $stmt_ubah_jumlah->bind_param("s", $id);
                $stmt_ubah_jumlah->execute();
                $result_ubah_jumlah = $stmt_ubah_jumlah->get_result();
            
                while ($r_ubah_jumlah = $result_ubah_jumlah->fetch_assoc()) {
                    $tahu = $r_ubah_jumlah['id_masakan'];
                    $tempe = $_POST['jumlah' . $tahu];
                    $id_pesan = $r_ubah_jumlah['id_pesan'];
            
                    $query_stok = "SELECT * FROM tb_masakan WHERE id_masakan = ?";
                    $stmt_stok = $conn->prepare($query_stok);
                    $stmt_stok->bind_param("s", $tahu);
                    $stmt_stok->execute();
                    $result_stok = $stmt_stok->get_result();
                    $row_stok = $result_stok->fetch_assoc();
                    $sisa_stok = $row_stok['stok'] - $tempe;
            
                    $query_proses_ubah = "UPDATE tb_pesan SET jumlah = ?, id_order = ? WHERE id_masakan = ? AND id_user = ? AND status_pesan != 'sudah'";
                    $stmt_proses_ubah = $conn->prepare($query_proses_ubah);
                    $stmt_proses_ubah->bind_param("ssss", $tempe, $id_ordernya, $tahu, $id);
                    $stmt_proses_ubah->execute();
            
                    $query_kurangi_stok = "UPDATE tb_masakan SET stok = ? WHERE id_masakan = ?";
                    $stmt_kurangi_stok = $conn->prepare($query_kurangi_stok);
                    $stmt_kurangi_stok->bind_param("ss", $sisa_stok, $tahu);
                    $stmt_kurangi_stok->execute();
            
                    $query_kelola_stok = "UPDATE tb_stok SET jumlah_terjual = ? WHERE id_pesan = ?";
                    $stmt_kelola_stok = $conn->prepare($query_kelola_stok);
                    $stmt_kelola_stok->bind_param("ss", $tempe, $id_pesan);
                    $stmt_kelola_stok->execute();
                }
            
                // Redirect if the order is saved successfully
                header('location: entri_order.php');
            } else {
                // Handle the case when no order is found
                echo "No order found for the user with ID: $id";
            }
